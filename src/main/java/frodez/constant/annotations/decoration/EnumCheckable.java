 ...
 * 2.要求:验证的枚举类必须实现一个方法(方法名建议为getIntroduction,如果有变动,请在使用LegalEnum注解时显式配置),<br>
 * 这个方法在当参数不符合要求时,返回枚举所对应的所有值,以字符串形式返回.</strong> <br>
 * 以下是示例代码(这里的getIntroduction方法由lombok自动生成,表现为introduction上有@Getter注解).<br>
 *
 * <pre>
