			...
			if (annotation != null) {
				if (!ClassUtils.isPrimitiveOrWrapper(parameter.getType())) {
					throw new CodeCheckException("方法", ReflectUtil.getFullMethodName(method), "的参数", parameter
						.getName(), "不是基本类型或者其装箱类,不能使用@", LegalEnum.class.getCanonicalName(), "注解");
				}
				checkLegalEnum(annotation);
			}
		}
	...
	private void checkLegalEnum(LegalEnum annotation) {
		try {
			...
			if (!enumClass.isAnnotationPresent(EnumCheckable.class)) {
				log.warn(StrUtil.concat(enumClassName, "被用于枚举类型检查,但是没有@", EnumCheckable.class.getCanonicalName(),
					"注解"));
			}
			...
			try {
				...
				throw new CodeCheckException(enumClassName, "未找到", methodName, "方法,或者方法入参类型有误,详情参见@",
					EnumCheckable.class.getCanonicalName(), "注解");
			}
